...
// Handle Background Notification payloads when app/browser is in background or closed
messaging.onBackgroundMessage((payload) => {
  ...
  const notificationOptions = {
    ...
    badge: '/logo192.jpeg',
    tag: payload.data?.tag || 'newlab-notification',
    data: {
      url: payload.data?.url || '/ar/results',
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = event.notification.data?.url || '/ar/results';

  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((windowClients) => {
      // Focus existing open window if present
